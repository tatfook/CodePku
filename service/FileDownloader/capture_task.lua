...
use the lib:
------------------------------------------------------------
NPL.load("(gl)Mod/CodePku/service/FileDownloader/capture_task.lua")
-------------------------------------------------------
]]
...
local CaptureRequest = commonlib.createtable(
    "Mod.CodePku.service.FileDownloader.localserver.CaptureRequest",
    {
...
local TaskManager = commonlib.createtable(
    "Mod.CodePku.service.FilDownloader.TaskManager",
    {
    ...
    elseif type(urls) == "table" then
        ...
        while url do
            if self.urls[url] then
                ...
                LOG.std(nil, "info", "CodePku", "warning: duplicate url removed from capture task:" .. url .. "\n")
            else
                self.urls[url] = true
                i = i + 1
            end
...
function Mod.CodePku.service.FileDownloader.localserver.ProcessURLRequest_result(request_id, index)
    ...
    if msg.code ~= 0 or msg.rcode ~= 200 then
        ...
        local ls = Mod.CodePku.service.FileDownloader.localserver.CreateStore(nil, 3)
...
-- @param request_id: request id
-- @param index: url index in the request
function Mod.CodePku.service.FileDownloader.localserver.ProcessWS_result(request_id, index)
    local task = TaskManager:GetTask(request_id)
    if not task then
...
-- @param request_id: request id
-- @param index: url index in the request
function Mod.CodePku.service.FileDownloader.localserver.ProcessFile_result(request_id, index)
    local task = TaskManager:GetTask(request_id)
...
local CaptureTask = commonlib.createtable(
    "Mod.CodePku.service.FilDownloader.CaptureTask",
    {
    ...
    if i <= num_urls then
        ...
        if not self.capture_request_.type then
            if System.localserver.UrlHelper.IsWebSerivce(url) then
                ...
                NPL.RegisterWSCallBack(
                    wsAddr,
                    string.format("Mod.CodePku.service.FileDownloader.localserver.ProcessWS_result(%d, %d)", self.capture_request_.id, i)
                )
                ...
                NPL.AsyncDownload(
                    ...
                    filename,
                    string.format("Mod.CodePku.service.FileDownloader.localserver.ProcessFile_result(%d, %d)", self.capture_request_.id, i),
        ...
        elseif self.capture_request_.type == 1 then
            ...
            NPL.AppendURLRequest(
                ...
                format(
                    "(%s)Mod.CodePku.service.FileDownloader.localserver.ProcessURLRequest_result(%d, %d)",
                    npl_thread_name,
