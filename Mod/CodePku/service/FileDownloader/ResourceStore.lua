...
Place: Foshan
------------------------------------------------------------
NPL.load("(gl)Mod/CodePku/service/FileDownloader/ResourceStore.lua")
local ResourceStore = commonlib.gettable("Mod.CodePku.service.FileDownloader.ResourceStore")
-------------------------------------------------------
]]
NPL.load("./localserver.lua")
NPL.load("./capture_task.lua")

local localserver = commonlib.gettable("Mod.CodePku.service.FileDownloader.localserver")
local TaskManager = commonlib.gettable("Mod.CodePku.service.FilDownloader.TaskManager")
local CaptureTask = commonlib.gettable("Mod.CodePku.service.FilDownloader.CaptureTask")
...
local ResourceStore = commonlib.inherit(
	commonlib.gettable("Mod.CodePku.service.FileDownloader.localserver"),
	commonlib.gettable("Mod.CodePku.service.FileDownloader.localserver.ResourceStore")
)
...
function ResourceStore:GetItem(url, info_only)
	...
	if not self.is_initialized_ then
		LOG.std(nil, "info", "CodePku", "warning: localserver instance uninitialized.\n")
		return
	end
	...
	if entry.payload_id == nil or entry.payload_id == 0 then
		LOG.std(nil, "info", "CodePku", "warning: ResourceStore item " .. url .. " are expected to have a payload. But it does not.\n")
		return
	end
