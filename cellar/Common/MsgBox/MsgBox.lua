...
function MsgBox:Show(msg, sec, overtimeMsg, witdh, height, index)
    ...
    local params = Mod.CodePku.Utils.ShowWindow(
        0,
        0,
        "Mod/CodePku/cellar/Common/MsgBox/MsgBox.html?msgId=" .. msgId .. "&width=" .. (witdh or 0) .. "&height=" .. (height or 0),
        "MsgBox",
        0,
        0,
        "_fi",
    ...
    params._page.OnClose = function()
        Mod.CodePku.Store:Remove("page/MsgBox" .. msgId)
    end

    Mod.CodePku.Utils.SetTimeOut(
        function()
            for key, item in ipairs(self.allMsgBox) do
                if item == msgId then
                    if overtimeMsg then
                        GameLogic.AddBBS(nil, overtimeMsg, 3000, "255 0 0", 21)
                    end
...
function MsgBox:Close(msgId)
    ...
    for key, value in ipairs(self.allMsgBox) do
        if value == msgId then
            MessageInfoPage = Mod.CodePku.Store:Get("page/MsgBox" .. msgId)
...
function MsgBox.SetPage(msgId)
    Mod.CodePku.Store:Set("page/MsgBox" .. msgId, document:GetPageCtrl())
end
...
function MsgBox:Dialog(content, customLabels, MsgBoxClick_CallBack, buttons, icon, isNotTopLevel, zorder)
    ...
    _guihelper.MessageBox(
        ...
        buttons,
        icon,
        "Mod/CodePku/cellar/Common/MsgBox/Dialog.html",
        isNotTopLevel,
        zorder
    )
